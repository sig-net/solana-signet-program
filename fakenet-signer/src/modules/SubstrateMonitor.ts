/* eslint-disable @typescript-eslint/no-explicit-any -- Substrate SCALE codecs
   and pallet extrinsics are dynamically typed; this monitor decodes them at
   the boundary. */
...
export class SubstrateMonitor {
  ...
  private stripScalePrefix(data: Uint8Array): Uint8Array {
    ...
    const mode = first & 0b11; // Check last 2 bits for mode
  ...
  async sendSignatureResponse(requestId: Uint8Array, signature: any) {
    const api = this.api;
    const keypair = this.keypair;
    if (!api) throw new Error('Not connected to Substrate');
    if (!keypair) throw new Error('No keypair available for signing');

    // Queue this transaction to prevent nonce conflicts
    return this.queueTransaction(async () => {
      try {
        // Create the transaction
        const respond = api.tx.signet?.respond;
        if (!respond) {
          throw new Error('signet.respond extrinsic unavailable');
        }
        const tx = respond([Array.from(requestId)], [signature]);
            ...
            if (status.isInBlock) {
              ...
              if (dispatchError) {
                if (dispatchError.isModule && this.api) {
                  ...
                  console.error(
                    `❌ Dispatch error: ${decoded.section}.${decoded.name}`
                  );
              ...
              console.error(
                `❌ Signature response failed with status: ${status.type}`
              );
  ...
  async sendRespondBidirectional(
    ...
    return this.queueTransaction(async () => {
      ...
      while (attempt <= maxRetries) {
        try {
          const respondBidirectional = api.tx.signet?.respondBidirectional;
          if (!respondBidirectional) {
            throw new Error(
              'signet.respondBidirectional extrinsic unavailable'
            );
          }
          const tx = respondBidirectional(
              ...
              if (status.isInBlock) {
                ...
                if (dispatchError) {
                  if (dispatchError.isModule && this.api) {
                    ...
                    console.error(
                      `❌ Dispatch error: ${decoded.section}.${decoded.name}: ${decoded.docs.join(' ')}`
                    );
                    reject(new Error(`${decoded.section}.${decoded.name}`));
                  } else {
                    console.error(
                ...
                console.error(
                  `❌ Transaction failed with status: ${status.type}`
                );
                reject(new Error(`Transaction ${status.type}`));
              }
            }).catch(reject);
          ...
          if (errorStr.includes('stale') && attempt < maxRetries) {
            attempt++;
            console.log(
              `⚠️ Stale nonce detected, retrying (attempt ${attempt}/${maxRetries})...`
            );
  ...
  async subscribeToEvents(handlers: {
    ...
    const systemEvents = this.api.query.system?.events;
    if (!systemEvents) {
      throw new Error('system.events query unavailable');
    }
    systemEvents((events: Vec<EventRecord>) => {
          ...
          switch (event.method) {
            ...
            case 'SignatureResponded':
              if (handlers.onSignatureResponded) {
                const [requestId, responder, signature] =
                  event.data as unknown as any[];
